...
public class storyMode extends JPanel 
{
    ...
    File inFile = new File("words.txt"); // word file for asteroids
...
//	gameStarted = new Date();
	...
	else if(died)  // checks using the died boolean whether you have los all of your lives
	{
		remove(userinput);
    ...
    class BallMover implements ActionListener 
    {
	 ...
	 asteroid = Toolkit.getDefaultToolkit().getImage("asteroid 2.png"); //getsimages for story mode
	    ...
	    else                              // checks for the levels(11 levels total)
	    {
		if(count==18)
		{
			lvl++;
     ...
     class bulletMover implements ActionListener {
        public void actionPerformed(ActionEvent e) //actionListener for the asteroids      // method that moves the projectile
        { // Controls the asteroid and randomizes words using math.random 
