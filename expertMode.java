...
public class expertMode extends JPanel 
{
    JFrame game;
    ...
    int x= 0;
    int time= 4; // time variable for timer for asteroid
 ...
 //   long speedHelper;
    ...
    public expertMode()
    {
	score = "";   // score variable that keeps track of the score based off of the number of asteroids shot
	died = congrats = false;
	lives = 1;
	life = "Lives: "+lives;
	...
	tankx1 = (this.getWidth()/20);
    	...
    	tankx3 = (this.getWidth()/2);                  // the four tanks being resized
    	tankx4 = (int)((this.getWidth()/1.35));
        spawn = new int[4];
	spawn[0] = tankx1;
	spawn[1] = tankx2;
	spawn[2] = tankx3;                     // spawning points of the tank put into an array
	spawn[3] = tankx4;
...
//	gameStarted = new Date();
	...
	//System.out.println(speedHelper);
        ...
        finalDestination = Toolkit.getDefaultToolkit().getImage("space4.jpg");           // get images for game
	...
	gameOverBoss = Toolkit.getDefaultToolkit().getImage("boss1.GIF");
       ...
       userinput.addKeyListener(k);    // textfield used for input from user
       add(userinput);
     
      
      detect = new Rectangle(ballX,ballY,170,168); // detect rectangle for collision detection
    ...
    public void getWords() { // reads 1000 words and puts them into an array
	...
	if(congrats) // checks using a boolean whether player has clearerd the game
	{
		remove(userinput);
	...
	else if(!(died))
	{
		...
		if(getspawn == 0)
		{
			...
			asteroidPlacer = tankx1;
        		...
        		Font ballFont = new Font("Arial",Font.PLAIN,26); //paints asteroids and words        g.setFont(ballFont);     // area where first asteroid is drawn
		...
		else if(getspawn == 1)
		{
			asteroidPlacer = tankx2;
        		g.drawImage(asteroid,asteroidPlacer, ballY,this.getWidth()/6, this.getHeight()/5,null);
        		Font ballFont = new Font("Arial",Font.PLAIN,26); //paints asteroids and words        g.setFont(ballFont);    // area where the second asteroid is drawn
        		g.drawString(word[x],(asteroidPlacer+40),(ballY+85));
		}
		else if(getspawn == 2)
		{
			asteroidPlacer = tankx3;
        		g.drawImage(asteroid,asteroidPlacer, ballY,this.getWidth()/6, this.getHeight()/5,null);
        		Font ballFont = new Font("Arial",Font.PLAIN,26); //paints asteroids and words        g.setFont(ballFont);  // area where the third asteroid is drawn
        		g.drawString(word[x],(asteroidPlacer+40),(ballY+85));
		}	
		else 
		{
			asteroidPlacer = tankx4;
        		g.drawImage(asteroid,asteroidPlacer, ballY,this.getWidth()/6, this.getHeight()/5,null);
        		Font ballFont = new Font("Arial",Font.PLAIN,26); //paints asteroids and words        g.setFont(ballFont);  // area where the fourth asteroid is drawn
        		g.drawString(word[x],(asteroidPlacer+40),(ballY+85));
		}
		if(asteroidHelper)
		{
			...
			if(bulletImage.contains(asteroidPlacer, ballY))    // checks of whether the the projectile has hit the asteroid
			{
				count++;
				balltimer.stop();
				bulletTimer.stop();
       		...
       		g.drawString(("Score:"+score),(this.getWidth()/20),50);
    ...
    class BallMover implements ActionListener 
    {
	 ...
	 asteroid = Toolkit.getDefaultToolkit().getImage("asteroid 2.png"); //getsimages for story mode
	    ...
	    if(ballY==paintGalaxy) // checks to make sure the asteroid han't crossed the home galaxy
	    {
			  life  = "Lives :"+(lives-1);
		lives--;
                if(lives == 0)
    ...
    class KeyText implements KeyListener      // class that reads input from the textfield using the enter key
    {
	String user2;
